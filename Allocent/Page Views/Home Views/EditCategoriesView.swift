                ...
                VStack(spacing: 16) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text("$\(viewModel.leftToBudget, specifier: "%.2f")")
                            .font(.title)
                            .fontWeight(.bold)
                            .foregroundStyle(viewModel.isOverAllocated ? .red : .primary)
                        ...
                        if viewModel.isOverAllocated {
                            HStack(spacing: 6) {
                                Image(systemName: "exclamationmark.triangle.fill")
                                    .foregroundStyle(.red)
                                    .font(.caption)
                                Text("You've allocated $\(abs(viewModel.leftToBudget), specifier: "%.2f") more than your income. Make sure you have savings to cover the difference.")
                                    .font(.caption)
                                    .foregroundStyle(.red)
                            }
                        }
                                ...
                                ForEach(viewModel.categories) { category in
                                    EditCategoryRow(
                                        ...
                                        onSave: { name, limit in
                                            Task { await viewModel.updateCategory(id: category.id, name: name, limit: limit, limitPercent: nil) }
                                        },
                                        onDelete: {
...
private struct AddCategorySheet: View {
    ...
    private func save() {
        ...
        isSaving = true
        Task {
            await viewModel.addCategory(name: category.rawValue, limit: 0.0, limitPercent: nil)
            await MainActor.run {
                isSaving = false
                isPresented = false
            }
